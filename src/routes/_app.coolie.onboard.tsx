...
import { UserPlus, Upload, BadgeCheck, Camera, Loader2, CheckCircle2, FileText } from "lucide-react";
...
async function uploadDoc(file: File, coolieId: string): Promise<string> {
  const fileName = `${coolieId}/${Date.now()}-${file.name}`;
  const { data, error } = await supabase.storage
    .from("onboarding-docs")
    .upload(fileName, file, { contentType: file.type, upsert: false });

  if (error) {
    console.warn("Doc upload failed:", error.message);
    return file.name;
  }
  return data.path;
}
...
function Onboard() {
  ...
  const addFiles = (files: FileList | null) => {
    ...
    setDocNames(prev => [...prev, ...arr.map(f => f.name)]);
  };
  ...
  const submit = async () => {
    ...
    try {
      ...
      let uploadedPaths: string[] = docNames;

      if (docFiles.length > 0) {
        uploadedPaths = await Promise.all(docFiles.map(f => uploadDoc(f, tempId)));
      }
      ...
      <Panel title="Coolie Onboarding Station" icon={<UserPlus className="h-6 w-6" />}>
        <div className="mb-6 flex items-center gap-2">
          {[1, 2, 3].map(n => (
            <div
              key={n}
              className={`h-1.5 flex-1 rounded-full transition ${step >= n ? "bg-gradient-gold" : "bg-maroon/60"}`}
            />
          ))}
        </div>
        ...
        {step === 1 && (
          <div className="space-y-4">
            ...
            <Field
              label="Badge / Billa Number"
              value={form.badge}
              onChange={v => setForm({ ...form, badge: v.toUpperCase() })}
              placeholder="NDLS-0421"
              icon={<BadgeCheck className="h-4 w-4 text-gold" />}
            />
            <button
              onClick={() => setStep(2)}
              disabled={!form.name || !form.badge}
        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            <label className="block cursor-pointer rounded-2xl border-2 border-dashed border-gold/40 bg-maroon/30 p-10 text-center transition hover:border-gold hover:bg-maroon/50">
              ...
              <p className="text-xs text-cream/60 mt-1">PDF, PNG, JPG · Max 5 MB each</p>
              <input
                type="file"
                multiple
            ...
            {docNames.length === 0 && (
              <button
                onClick={() => setDocNames(["aadhaar-front.pdf", "police-verification.pdf"])}
              ...
              <div className="space-y-2">
                {docNames.map((d, i) => (
                  <div key={i} className="flex items-center gap-2 rounded-lg bg-maroon/40 px-3 py-2 text-sm text-cream">
                    ...
                    {docFiles[i] && (
                      <span className="ml-auto text-[10px] text-green-300 uppercase tracking-widest">Ready to upload</span>
                    )}
                  </div>
                ))}
              </div>
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => setStep(3)}
                disabled={docNames.length === 0}
                className="flex-1 rounded-xl bg-gradient-gold py-3 font-semibold text-maroon disabled:opacity-40"
              >
                Next: Profile Photo
              </button>
            </div>
        ...
        {step === 3 && (
          <div className="space-y-5">
            ...
            <div className="flex justify-center gap-2">
              {AVATARS.map(a => (
                <button
                  key={a}
                  onClick={() => setAvatar(a)}
                  className={`flex h-12 w-12 items-center justify-center rounded-full text-2xl transition ${avatar === a ? "bg-gradient-gold scale-110" : "bg-maroon/40 hover:bg-maroon/60"}`}
                >
                  {a}
                </button>
              ))}
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={submit}
                disabled={!avatar || submitting}
                className="flex-1 inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-gold py-3 font-semibold text-maroon disabled:opacity-40"
              >
                {submitting ? (
                  <>
  ...
  return (
    <div className="flex min-h-[60vh] items-center justify-center">
      <motion.div
        ...
        className="glass-gold max-w-md p-10 text-center"
      >
        <motion.div
          animate={{ rotate: 360 }}
        ...
        <p className="mt-6 text-xs text-cream/50">
          Tip: switch to <strong className="text-gold">Admin</strong> role from the top bar to approve yourself for testing.
        </p>
      </motion.div>
    </div>
  );
